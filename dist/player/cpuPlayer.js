...
export class CPUPlayer {
    ...
    gameState;
    difficulty;
    constructor(name, gameState, difficulty) {
        this.name = name;
    ...
    async takeTurn(currentString) {
        ...
        switch (this.difficulty) {
            ...
            default:
                throw new Error("This state should never be reached: invalid difficulty level");
        }
    }
    async respondToChallenge(currentString) {
        ...
        const word = possibleWords.length === 0
    ...
    getWordsContainingString(currentString, wordSet = this.gameState.getWordSet()) {
        const possibleWords = [...wordSet].filter((word) => word.includes(currentString));
        return possibleWords;
    }
    addLetterEasy(currentString, possibleWords) {
        ...
        const nonKillingWords = possibleWords.filter((word) => word.length % numPlayers !== unwantedParity);
        const pursuedWord = nonKillingWords.length === 0
            ? possibleWords[Math.floor(Math.random() * possibleWords.length)]
    ...
    computeProbabilityBadWord(candidateString, candidateWords) {
        const alphabet = "abcdefghijklmnopqrstuvwxyz";
        const numPlayers = this.gameState.getNumPlayers();
        ...
        while (queue.length !== 0) {
            ...
            for (const [prevProbability, currString, currWords] of queue) {
                ...
                for (let position of [Position.START, Position.END]) {
                    for (let letter of alphabet) {
                        const nextString = position === Position.START
                            ? letter + currString
                            : currString + letter;
                        const nextWords = this.getWordsContainingString(nextString, currWords);
                        // Number of words that the current player would NOT complete
                        const numGood = nextWords.filter((word) => word.length % numPlayers !== currPlayerIndex).length;
                        scores.push(numGood);
                ...
                scores.forEach((value, index) => {
                    ...
                    if (value !== 0) {
                        ...
                        const nextSubstring = index < alphabet.length
                            ? letter + currString
                            : currString + letter;
                        nextQueue.push([
    ...
    addLetterMedium(currentString, possibleWords) {
        ...
        for (let position of [Position.START, Position.END]) {
            for (let letter of alphabet) {
                ...
                if (this.gameState.getWordSet().has(candidateString)) {
                    ...
                    const candidateWords = this.getWordsContainingString(candidateString, possibleWords);
                    scores.push(1.0 -
        ...
        const position = selectedIndex < alphabet.length ? Position.START : Position.END;
        return { letter: letter, position: position };
    }
}
